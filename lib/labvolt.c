...
void shut_down(void)					//Disables motors and closes connection to device
{
	DLLDisableMotors();
	DLLCloseDevice();
}
...
{
	unsigned char i;
	//bcrem  DLLSetDesiredPosition(0x00,-GPCLOSE);
...
void gripperOpen(void)                                 //Open the gripper
{
	...
	DLLSetDesiredPosition(0x00,GPOPEN);
	DLLRunMotors();
	j = 0;
	do {
		DLLMotorRunStatus(&i);
